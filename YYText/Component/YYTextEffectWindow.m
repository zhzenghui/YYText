...
- (CGFloat)_updateMagnifier:(YYTextMagnifier *)mag {
    ...
    if (mag.captureDisabled) {
        if (!mag.snapshot || mag.snapshot.size.width > 1) {
            ...
            dispatch_once(&onceToken, ^{
                ...
                fmt.opaque = NO;
                UIGraphicsImageRenderer *renderer = [[UIGraphicsImageRenderer alloc] initWithSize:rect.size format:fmt];
    ...
    fmt.scale = [UIScreen mainScreen].scale;
    fmt.opaque = NO;
    UIGraphicsImageRenderer *renderer = [[UIGraphicsImageRenderer alloc] initWithSize:captureRect.size format:fmt];
    __block UIImage *capturedImage = nil;
    capturedImage = [renderer imageWithActions:^(UIGraphicsImageRendererContext *rendererContext) {
